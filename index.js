...
function makePersonObject(personId, personName, personEmail) {
  const personObject = { id: personId, name: personName, email: personEmail };
  return personObject;
}
console.log(makePersonObject(7, "Mark", "mark@SpeechGrammarList.com"));
...
console.log(getName({ name: "Leia", id: 1, email: "leia@leia.com" }));
...
console.log(appleIndex(["orange", "grape", "apple"]));
/**
...
// }
// console.log(getCarInfoByIndex(inventory, 12));
...
function getCarInfoById(inventory, index) {
  return `This is a ${inventory[index - 1].car_make} ${
    inventory[index - 1].car_model
  }`;
}
 ...
 *
 * @instructions
 * We need a tool to find older cars!
...
function getOlderCars(inventory, maxYear) {
  let newArray = [];
  for (let i = 0; i < inventory.length; i++) {
    if (inventory[i].car_year < maxYear) newArray.push(inventory[i].car_year);
  }
  return newArray;
}
console.log(getOlderCars(inventory, 1990));
...
//   return newArray;
// }
// console.log(getGermanCars("Audi"));
